 ...
 * changing the name of PyInit_person and attempting the import).
 *
 ...
 *
 * >>> import person
 * >>> p = person.Person(first_name="Isaac", last_name="Newton", number=42)
...
static PyObject *Person_str(struct Person *self)
{
...
static enum PythonVersionDifference version_difference(void){
    ...
    if(major != PY_MAJOR_VERSION){
        printf("person was built for version '%d' but is being run by python '%ld'\n", \
                PY_MAJOR_VERSION, major);
        return DIFFERENT_MAJOR;
    }
    if(minor != PY_MINOR_VERSION){
        printf("person was built for version '%d.%d' but is being run by python '%ld.%ld'\n",
...
PyMODINIT_FUNC PyInit_person(void)
{
    switch(version_difference()){
        case DIFFERENT_MAJOR: // the dlopen call in CPython will probably miss some symbols and fail
                              ...
                              printf("person: Different major versions: For sure that won't work!\n");
                              return NULL;
        case DIFFERENT_MINOR: // As far as I know, the API is stable enough that a minor version difference
                              ...
                              printf("person: different python versions not sure what the implication is\n");
                              break;
        case SAME:
                              break;
    }
    ...
    PyObject *m = PyModule_Create(&personmodule);
    if(m == NULL){
        return NULL;
    }
